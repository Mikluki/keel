...
with prose in `bodies/<id>.md` - the graph is the only thing you read and write.

The `*.view.md` files are INVISIBLE to you. They are the human's private live-preview -
gitignored, regenerated by their own `keel watch` daemon or editor hook. Never read one,
render one, run `view`/`watch`, check whether one re-rendered, or mention one in a status
report. Your surfaces are the graph files and `--toon` output; how the human previews the
graph is entirely their concern, off your critical path.
